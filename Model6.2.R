...
Data<-Data_Wins%>%
  select(DtD, Scope12, Scp12Int, Scp12YoY, Disclosure, DisclosureYoY, Leverage, Size, Profitability, Liquidity, EquityBuffer, MarketReturn , MarketVolatility , Inflation , RiskFree, Year.y.y, Sector_number, Country_number, Sector, Firm)
...
for (i in 1:max_sector) {
  ...
  # Model 1
  model1 <- lm(DtD ~ Scope12*Disclosure + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility + RiskFree + Year.y.y + Country_number, data = SectorData)
  
  # Model 2
  model2 <- lm(DtD ~ Scp12Int*Disclosure + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility + RiskFree + Year.y.y + Country_number, data = SectorData)
  
  # Model 3
  model3 <- lm(DtD ~ Scp12YoY*DisclosureYoY + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility + RiskFree + Year.y.y + Country_number, data = SectorData)
  ...
  writeLines(output, paste0("6.2_",Sector_name, "_models.tex"))
...
results_df1 <- results_df1 %>%
  ...
  select(Sector,Scope12, `Scope12:Disclosure1`, Disclosure1)
...
# Save output in a text file
writeLines(summary1, paste0("6.21_Summary.tex"))
...
results_df2 <- results_df2 %>%
  ...
  select(Sector,Scp12Int, `Scp12Int:Disclosure1`, Disclosure1)
...
# Save output in a text file
writeLines(summary2, paste0("6.22_Summary.tex"))
...
results_df3 <- results_df3 %>%
  ...
  select(Sector,Scp12YoY, `Scp12YoY:DisclosureYoY1`, DisclosureYoY1)
...
# Save output in a text file
writeLines(summary3, paste0("6.23_Summary.tex"))