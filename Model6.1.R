...
Data<-Data_Wins%>%
  select(DtD, Scope12, Scp12Int, Scp12YoY, Leverage, Size, Profitability, Liquidity, EquityBuffer, MarketReturn , MarketVolatility , Inflation , RiskFree, Year.y.y, Sector_number, Country_number, Sector, Firm)
...
for (i in 1:max_sector) {
  ...
  # Ensure Sector_name is a single value
  Sector_name <- unique(Sector_mapping$Sector[Sector_mapping$Sector_number == i])
  if(length(Sector_name) != 1) {
    next  # Skip this iteration if no unique sector name
  }
  
  # Fit models
  model1 <- lm(DtD ~ Scope12 + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility + RiskFree + Year.y.y + Country_number, data = SectorData)
  model2 <- lm(DtD ~ Scp12Int + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility + RiskFree + Year.y.y + Country_number, data = SectorData)
  model3 <- lm(DtD ~ Scp12YoY + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility + RiskFree + Year.y.y + Country_number, data = SectorData)
...
writeLines(output, paste0("6.1_",i,Sector_name, "_models.tex"))
...
results_wide <- results_wide %>%
  ...
  select(Sector, Scope12, Scp12Int, Scp12YoY)
...
# Save output in a text file
writeLines(summary, paste0("6.1_Summary.tex"))