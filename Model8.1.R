...
for (i in 1:max_year_group) {
  ...
  # Model 1
  model1 <- lm(DtD ~ Scope12 + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility + RiskFree + Year.y.y + sector_country, data = YearGroupData)
  
  # Model 2
  model2 <- lm(DtD ~ Scp12Int + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility + RiskFree + Year.y.y + sector_country, data = YearGroupData)
  
  # Model 3
  model3 <- lm(DtD ~ Scp12YoY + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility  + RiskFree + Year.y.y + sector_country, data = YearGroupData)
  ...
  writeLines(output, paste0("8.1_","Year_Group_", i, "_models.tex"))
...
# Save output in a text file
writeLines(summary, paste0("8.1_Summary.tex"))
