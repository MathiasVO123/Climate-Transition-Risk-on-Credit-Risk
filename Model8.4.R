...
Data <- Data %>%
  mutate(year_group = case_when(
    Year.y.y >= 2010 & Year.y.y <= 2012 ~ 0,
    Year.y.y >= 2013 & Year.y.y <= 2015 ~ 1,
    Year.y.y >= 2016 & Year.y.y <= 2018 ~ 2,
    Year.y.y >= 2019 & Year.y.y <= 2022 ~ 3,
  ))
...
for (i in 1:max_year_group) {
  ...
  # Model 1
  model1 <- lm(DtD ~ Scope12*SBTiTarget + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility  + RiskFree + Year.y.y + sector_country, data = YearGroupData)
  
  # Model 2
  model2 <- lm(DtD ~ Scp12Int*SBTiTarget + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility  + RiskFree + Year.y.y + sector_country, data = YearGroupData)
  
  # Model 3
  model3 <- lm(DtD ~ Scp12YoY*SBTiTarget + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility + RiskFree + Year.y.y + sector_country, data = YearGroupData)
  
  # Adjust standard errors for clustering at the firm level
  model1_results <- coeftest(model1, vcov = vcovCL(model1, cluster = ~Firm, data = YearGroupData))
  model2_results <- coeftest(model2, vcov = vcovCL(model2, cluster = ~Firm, data = YearGroupData))
  model3_results <- coeftest(model3, vcov = vcovCL(model3, cluster = ~Firm, data = YearGroupData))
  ...
  writeLines(output, paste0("8.4_","Year_Group_", i, "_models.tex"))
  ...
  coef_df3 <- coef_df3 %>%
    filter(term %in% c("Scp12YoY", "SBTiTarget1","Scp12YoY:SBTiTarget1")) %>%
    mutate(Significance = case_when(
      p.value < 0.01 ~ "***",
      p.value < 0.05  ~ "**",
      p.value < 0.10  ~ "*",
  ...
  # Append results to the summary dataframe
  for (var in c("Scp12YoY", "SBTiTarget1","Scp12YoY:SBTiTarget1")) {
...
results_df1 <- results_df1 %>%
  ...
  select(Year,Scope12, `Scope12:SBTiTarget1`, SBTiTarget1)
...
# Save output in a text file
writeLines(summary1, paste0("8.41_Summary.tex"))
...
results_df2 <- results_df2 %>%
  ...
  select(Year,Scp12Int, `Scp12Int:SBTiTarget1`, SBTiTarget1)
...
# Save output in a text file
writeLines(summary2, paste0("8.42_Summary.tex"))
...
results_df3 <- results_df3 %>%
  mutate(
    ...
    `Scp12YoY:SBTiTarget1` = ifelse(!is.na(`Coefficient_Scp12YoY:SBTiTarget1`), paste0(sprintf("%.3f", `Coefficient_Scp12YoY:SBTiTarget1`), `Significance_Scp12YoY:SBTiTarget1`), NA),
    SBTiTarget1 = ifelse(!is.na(Coefficient_SBTiTarget1), paste0(sprintf("%.3f", Coefficient_SBTiTarget1), Significance_SBTiTarget1), NA)
  ) %>%
  select(Year,Scp12YoY, `Scp12YoY:SBTiTarget1`, SBTiTarget1)
...
# Save output in a text file
writeLines(summary3, paste0("8.43_Summary.tex"))