...
Data<-Data_Wins%>%
  select(DtD, Scope12, Scp12Int, Scp12YoY, Disclosure, DisclosureYoY, Audit, AuditYoY, Leverage, Size, Profitability, Liquidity, EquityBuffer, MarketReturn , MarketVolatility , Inflation , RiskFree, Year.y.y, Sector_number, Country_number, Firm)
...
#Model 1
model1 <- lm(DtD ~ Scope12*Audit + Leverage + log(Size)+ Profitability+ Liquidity + EquityBuffer+ MarketReturn + MarketVolatility + RiskFree + Year.y.y + sector_country, data = Data)
summary(model1)

#Model 2
model2 <- lm(DtD ~ Scp12Int*Audit + Leverage + log(Size)+ Profitability+ Liquidity+ EquityBuffer+ MarketReturn + MarketVolatility + RiskFree  + Year.y.y + sector_country, data = Data)
summary(model2)

#Model 3
model3 <- lm(DtD ~ Scp12YoY*AuditYoY + Leverage + log(Size)+ Profitability+ Liquidity+ EquityBuffer+ MarketReturn + MarketVolatility + RiskFree  + Year.y.y + sector_country, data = Data)
summary(model3)
...
ggsave("residual_plot13.png", plot = residual_plot1, width = 6, height = 3, dpi = 300)
...
ggsave("residual_plot23.png", plot = residual_plot2, width = 6, height = 3, dpi = 300)
...
ggsave("residual_plot33.png", plot = residual_plot3, width = 6, height = 3, dpi = 300)
...
ggsave("Normal13.png", combined_plot1, width = 12, height = 6)
...
# Save the combined plot with appropriate dimensions
ggsave("Normal23.png", combined_plot2, width = 12, height = 6)
...
# Save the combined plot with appropriate dimensions
ggsave("Normal33.png", combined_plot3, width = 12, height = 6)
