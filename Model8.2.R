...
for (i in 1:max_year_group) {
  ...
  # Model 3
  model3 <- lm(DtD ~ Scp12YoY*DisclosureYoY + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility + Inflation + RiskFree + Year.y.y + sector_country, data = YearGroupData)
  ...
  writeLines(output, paste0("8.2_","Year_Group_", i, "_models.tex"))
...
results_df1 <- results_df1 %>%
  ...
  select(Year,Scope12, `Scope12:Disclosure1`, Disclosure1)
...
# Save output in a text file
writeLines(summary1, paste0("8.21_Summary.tex"))
...
results_df2 <- results_df2 %>%
  ...
  select(Year,Scp12Int, `Scp12Int:Disclosure1`, Disclosure1)
...
# Save output in a text file
writeLines(summary2, paste0("8.22_Summary.tex"))
...
results_df3 <- results_df3 %>%
  ...
  select(Year,Scp12YoY, `Scp12YoY:DisclosureYoY1`, DisclosureYoY1)
...
# Save output in a text file
writeLines(summary3, paste0("8.23_Summary.tex"))