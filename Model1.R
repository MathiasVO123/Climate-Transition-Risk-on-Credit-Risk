...
Data<-Data_Wins%>%
  select(DtD, Scope12, Scp12Int, Scp12YoY, Size, Leverage, Profitability, Liquidity, EquityBuffer, MarketReturn , MarketVolatility , Inflation , RiskFree, Year.y.y, Sector_number, Country_number, Firm)
...
#Base models
model1 <- lm(DtD ~ Scope12 + log(Size)+ Leverage+ Profitability+ Liquidity + EquityBuffer+ MarketReturn + MarketVolatility + RiskFree + Year.y.y + sector_country, data = Data)
model2 <- lm(DtD ~ Scp12Int + log(Size)+ Leverage+Profitability+ Liquidity+ EquityBuffer+ MarketReturn + MarketVolatility  + RiskFree  + Year.y.y + sector_country, data = Data)
model3 <- lm(DtD ~ Scp12YoY + log(Size)+ Leverage+Profitability+ Liquidity+ EquityBuffer+ MarketReturn + MarketVolatility + RiskFree  + Year.y.y + sector_country, data = Data)
...
stargazer(model1, model2, model3, type = "latex",
          ...
          add.lines = list(c("Country&Sector fixed-effects", "Yes", "Yes", "Yes", "Yes", "Yes"), 
...
ggsave("residual_plot1.png", plot = residual_plot1, width = 6, height = 3, dpi = 300)
...
ggsave("residual_plot2.png", plot = residual_plot2, width = 6, height = 3, dpi = 300)
...
ggsave("residual_plot3.png", plot = residual_plot3, width = 6, height = 3, dpi = 300)
...
#Rank
DataRank1<-Data%>%
  select(Scope12, Size, Profitability, Liquidity, EquityBuffer, MarketReturn , MarketVolatility , RiskFree)%>%
  na.omit()
...
print(Rank1, include.rownames = FALSE)
...
##Model 2
DataRank2<-Data%>%
  select(Scp12Int, Size, Profitability, Liquidity, EquityBuffer, MarketReturn , MarketVolatility , RiskFree)%>%
  na.omit()
...
print(Rank2, include.rownames = FALSE)
...
##Model 3
DataRank3<-Data%>%
  select(Scp12YoY, Size, Profitability, Liquidity, EquityBuffer, MarketReturn , MarketVolatility , RiskFree)%>%
  na.omit()
...
print(Rank3, include.rownames = FALSE)
...
independent_vars <- setdiff(all.vars(formula(model1)), c(as.character(formula(model1)[[2]]), "Year.y.y", "sector_country"))
cor_res1<- Data[, c("residuals1", independent_vars)]
corres_matrix1 <- cor(cor_res1, use = "complete.obs")
...
ggsave("Normal1.png", combined_plot1, width = 12, height = 6)
...
# Save the combined plot with appropriate dimensions
ggsave("Normal2.png", combined_plot2, width = 12, height = 6)
...
# Save the combined plot with appropriate dimensions
ggsave("Normal3.png", combined_plot3, width = 12, height = 6)
