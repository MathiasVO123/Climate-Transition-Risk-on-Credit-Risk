...
SP_all$Firm<-as.numeric(SP_all$Firm)

Data<-Data_Wins%>%
  select(Date, DtD, Scope12, Scp12Int, Scp12YoY, HYRating, Leverage, Size, Profitability, Liquidity, EquityBuffer, MarketReturn , MarketVolatility , Inflation , RiskFree, Year.y.y, Sector_number, Country_number, Firm)%>%
  filter(!is.na(HYRating)) 

DataR<-Data%>%
  full_join(SP_all, by=c("Firm","Date"))%>%
  na.omit()%>%
  distinct()

# Summarize by Rating
rating_summary <- DataR %>%
  group_by(Rating) %>%
...
Data$HYRating<-as.factor(Data$HYRating)
...
#Model 1
model1 <- lm(DtD ~ Scope12*HYRating + Leverage + log(Size)+ Profitability+ Liquidity + EquityBuffer+ MarketReturn + MarketVolatility + RiskFree + Year.y.y + sector_country, data = Data)
summary(model1)

#Model 2
model2 <- lm(DtD ~ Scp12Int*HYRating + Leverage + log(Size)+ Profitability+ Liquidity+ EquityBuffer+ MarketReturn + MarketVolatility + RiskFree  + Year.y.y + sector_country, data = Data)
summary(model2)

#Model 3
model3 <- lm(DtD ~ Scp12YoY*HYRating + Leverage + log(Size)+ Profitability+ Liquidity+ EquityBuffer+ MarketReturn + MarketVolatility + RiskFree  + Year.y.y + sector_country, data = Data)
summary(model3)
...
#Rank
DataRank1<-Data%>%
  select(Scope12, HYRating, Size, Profitability, Liquidity, EquityBuffer, MarketReturn , MarketVolatility , Inflation , RiskFree)%>%
  na.omit()
...
##Model 2
DataRank2<-Data%>%
  select(Scp12Int, Size, Profitability, Liquidity, EquityBuffer, MarketReturn , MarketVolatility , Inflation , RiskFree)%>%
  na.omit()
...
##Model 3
DataRank3<-Data%>%
  select(Scp12YoY, Size, Profitability, Liquidity, EquityBuffer, MarketReturn , MarketVolatility , Inflation , RiskFree)%>%
  na.omit()
