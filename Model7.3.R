...
Data<-Data_Wins%>%
  select(DtD, Scope12, Scp12Int, Scp12YoY, Leverage, Disclosure, Audit, AuditYoY, Size, Profitability, Liquidity, EquityBuffer, MarketReturn , MarketVolatility , Inflation , RiskFree, Year.y.y, Sector_number, Country_number, Country, Firm)
...
for (i in 1:max_Country) {
  ...
  # Model 1
  model1 <- lm(DtD ~ Scope12*Audit + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility + RiskFree + Year.y.y + Sector_number, data = CountryData)
  
  # Model 2
  model2 <- lm(DtD ~ Scp12Int*Audit + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility + RiskFree + Year.y.y + Sector_number, data = CountryData)
  
  # Model 3
  model3 <- lm(DtD ~ Scp12YoY*AuditYoY + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility + RiskFree + Year.y.y + Sector_number, data = CountryData)
  ...
  writeLines(output, paste0("7.3_",country_name, "_models.tex"))
...
results_df1 <- results_df1 %>%
  ...
  select(Country,Scope12, `Scope12:Audit1`, Audit1)
...
# Save output in a text file
writeLines(summary1, paste0("7.31_Summary.tex"))
...
results_df2 <- results_df2 %>%
  ...
  select(Country,Scp12Int, `Scp12Int:Audit1`, Audit1)
...
# Save output in a text file
writeLines(summary2, paste0("7.32_Summary.tex"))
...
results_df3 <- results_df3 %>%
  ...
  select(Country,Scp12YoY, `Scp12YoY:AuditYoY1`, AuditYoY1)
...
# Save output in a text file
writeLines(summary3, paste0("7.33_Summary.tex"))