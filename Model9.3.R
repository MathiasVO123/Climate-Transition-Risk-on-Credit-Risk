...
Data<-Data_Wins%>%
  select(DtD, Scope12, Scp12Int, Scp12YoY, Disclosure, Audit, Leverage, Size, Profitability, Liquidity, EquityBuffer, MarketReturn , MarketVolatility , Inflation , RiskFree, Year.y.y, Sector_number, Country_number, Country, Firm)
...
for (i in 1:max_year_group) {
  ...
  # Model 1
  model1 <- lm(DtD ~ Scope12*Disclosure*Audit + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility + Inflation + RiskFree + Year.y.y + sector_country, data = YearGroupData)
  
  # Model 2
  model2 <- lm(DtD ~ Scp12Int*Disclosure*Audit + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility + Inflation + RiskFree + Year.y.y + sector_country, data = YearGroupData)
  
  # Model 3
  model3 <- lm(DtD ~ Scp12YoY*Disclosure*Audit + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility + Inflation + RiskFree + Year.y.y + sector_country, data = YearGroupData)
  ...
  writeLines(output, paste0("9.3_","Year_Group_", i, "_models.tex"))
  ...
  coef_df <- coef_df %>%
    filter(term %in% c("Audit","Scope12:Audit","Scp12Int:Audit","Scp12YoY:Audit")) %>%
  ...
  # Append results to the summary dataframe
  for (var in c("Scope12:Audit","Scp12Int:Audit","Scp12YoY:Audit")) {
    if (any(coef_df$term == var)) {  # Check if variable exists in the model output
...
for (i in 1:max_year_group) {
  ...
  # After processing models, assume results_df1 now contains all Audit entries for this Year
  
  # Get indices of Audit entries for this Year in results_df1
  Indicies <- which(results_df1$Year == i & results_df1$Variable == "Audit")
  
  # Rename the Audit entries to Audit1, Audit2, Audit3
  if (length(Indicies) == 3) {  # Ensure there are exactly three entries as expected
    results_df1$Variable[Indicies] <- paste("Audit", 1:3, sep = "")
  }
}
...
results_df1 <- results_df1 %>%
  mutate(
    Audit1 = ifelse(!is.na(Coefficient_Audit1), paste0(sprintf("%.2f", Coefficient_Audit1), Significance_Audit1), NA),
    Audit2 = ifelse(!is.na(Coefficient_Audit2), paste0(sprintf("%.2f", Coefficient_Audit2), Significance_Audit2), NA),
    Audit3 = ifelse(!is.na(Coefficient_Audit3), paste0(sprintf("%.2f", Coefficient_Audit3), Significance_Audit3), NA)
  ) %>%
  select(Year,Audit1, Audit2, Audit3)
...
# Save output in a text file
writeLines(summary1, paste0("9.31_Summary.tex"))
...
# Combine coefficient and significance into one string
colnames(results_df2) <- sub(":Audit", "", colnames(results_df2))
...
# Save output in a text file
writeLines(summary2, paste0("9.32_Summary.tex"))
