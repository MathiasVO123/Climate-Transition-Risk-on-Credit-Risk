...
Data<-Data_Wins%>%
  select(DtD, Scope12, Scp12Int, Scp12YoY, Disclosure, DisclosureYoY, Leverage, Size, Profitability, Liquidity, EquityBuffer, MarketReturn , MarketVolatility , Inflation , RiskFree, Year.y.y, Sector_number, Country_number, Firm)
...
#Model 1
model1 <- lm(DtD ~ Scope12*Disclosure + Leverage + log(Size)+ Profitability+ Liquidity + EquityBuffer+ MarketReturn + MarketVolatility + RiskFree + Year.y.y + sector_country, data = Data)
summary(model1)

#Model 2
model2 <- lm(DtD ~ Scp12Int*Disclosure + Leverage + log(Size)+ Profitability+ Liquidity+ EquityBuffer+ MarketReturn + MarketVolatility + RiskFree  + Year.y.y + sector_country, data = Data)
summary(model2)

#Model 3
model3 <- lm(DtD ~ Scp12YoY*DisclosureYoY + Leverage + log(Size)+ Profitability+ Liquidity+ EquityBuffer+ MarketReturn + MarketVolatility + RiskFree  + Year.y.y + sector_country, data = Data)
summary(model3)
...
ggsave("residual_plot12.png", plot = residual_plot1, width = 6, height = 3, dpi = 300)
...
ggsave("residual_plot22.png", plot = residual_plot2, width = 6, height = 3, dpi = 300)
...
ggsave("residual_plot32.png", plot = residual_plot3, width = 6, height = 3, dpi = 300)
...
#Rank
DataRank1<-Data%>%
  select(Scope12, Disclosure, Size, Profitability, Liquidity, EquityBuffer, MarketReturn , MarketVolatility , RiskFree)%>%
  na.omit()
...
##Model 2
DataRank2<-Data%>%
  select(Scp12Int, Disclosure, Size, Profitability, Liquidity, EquityBuffer, MarketReturn , MarketVolatility , RiskFree)%>%
  na.omit()
...
##Model 3
DataRank3<-Data%>%
  select(Scp12YoY, Disclosure, Size, Profitability, Liquidity, EquityBuffer, MarketReturn , MarketVolatility  , RiskFree)%>%
  na.omit()
...
ggsave("Normal12.png", combined_plot1, width = 12, height = 6)
...
# Save the combined plot with appropriate dimensions
ggsave("Normal22.png", combined_plot2, width = 12, height = 6)
...
# Save the combined plot with appropriate dimensions
ggsave("Normal32.png", combined_plot3, width = 12, height = 6)
