...
Data<-Data_Wins%>%
  select(DtD, Scope12, Scp12Int, Scp12YoY, Disclosure, Audit, AuditYoY, Leverage, Size, Profitability, Liquidity, EquityBuffer, MarketReturn , MarketVolatility , Inflation , RiskFree, Year.y.y, Sector_number, Country_number, Country, Firm)

Data <- Data %>%
  mutate(year_group = case_when(
    Year.y.y >= 2010 & Year.y.y <= 2012 ~ 1,
    Year.y.y >= 2013 & Year.y.y <= 2015 ~ 2,
    Year.y.y >= 2016 & Year.y.y <= 2018 ~ 3,
    Year.y.y >= 2019 & Year.y.y <= 2022 ~ 4,
  ))
...
for (i in 1:max_year_group) {
  ...
  # Model 1
  model1 <- lm(DtD ~ Scope12*Audit + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility + RiskFree + Year.y.y + sector_country, data = YearGroupData)
  
  # Model 2
  model2 <- lm(DtD ~ Scp12Int*Audit + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility  + RiskFree + Year.y.y + sector_country, data = YearGroupData)
  
  # Model 3
  model3 <- lm(DtD ~ Scp12YoY*AuditYoY + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility  + RiskFree + Year.y.y + sector_country, data = YearGroupData)
  ...
  writeLines(output, paste0("8.3_","Year_Group_", i, "_models.tex"))
...
results_df1 <- results_df1 %>%
  ...
  select(Year,Scope12, `Scope12:Audit1`, Audit1)
...
# Save output in a text file
writeLines(summary1, paste0("8.31_Summary.tex"))
...
results_df2 <- results_df2 %>%
  ...
  select(Year,Scp12Int, `Scp12Int:Audit1`, Audit1)
...
# Save output in a text file
writeLines(summary2, paste0("8.32_Summary.tex"))
...
results_df3 <- results_df3 %>%
  ...
  select(Year,Scp12YoY, `Scp12YoY:AuditYoY1`, AuditYoY1)
...
# Save output in a text file
writeLines(summary3, paste0("8.33_Summary.tex"))