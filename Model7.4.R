...
Data<-Data_Wins%>%
  select(DtD, Scope12, Scp12Int, Scp12YoY, SBTiTarget, SBTiTargetYoY, Leverage, Size, Profitability, Liquidity, EquityBuffer, MarketReturn , MarketVolatility , Inflation , RiskFree, Year.y.y, Sector_number, Country_number, Country, Firm)
...
Data$SBTiTargetYoY<-as.factor(Data$SBTiTargetYoY)
...
for (i in 1:max_Country) {
  ...
  # Model 1
  model1 <- lm(DtD ~ Scope12*SBTiTarget + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility + RiskFree + Year.y.y + Sector_number, data = CountryData)
  
  # Model 2
  model2 <- lm(DtD ~ Scp12Int*SBTiTarget + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility + RiskFree + Year.y.y + Sector_number, data = CountryData)
  
  # Model 3
  model3 <- lm(DtD ~ Scp12YoY*SBTiTargetYoY + Leverage + log(Size) + Profitability + Liquidity + EquityBuffer + MarketReturn + MarketVolatility + RiskFree + Year.y.y + Sector_number, data = CountryData)
  ...
  writeLines(output, paste0("7.4_",country_name, "_models.tex"))
  ...
  coef_df1 <- tidy1 %>%
    filter(term %in% c("Scope12", "SBTiTarget1", "Scope12:SBTiTarget1")) %>%
  ...
  coef_df2 <- coef_df2 %>%
    filter(term %in% c("Scp12Int", "SBTiTarget1", "Scp12Int:SBTiTarget1")) %>%
    mutate(Significance = case_when(
      p.value < 0.01 ~ "***",
      p.value < 0.05  ~ "**",
      p.value < 0.10  ~ "*",
  ...
  coef_df3 <- coef_df3 %>%
    filter(term %in% c("Scp12YoY", "SBTiTargetYoY1","Scp12YoY:SBTiTargetYoY1")) %>%
    mutate(Significance = case_when(
      p.value < 0.01 ~ "***",
      p.value < 0.05  ~ "**",
      p.value < 0.10  ~ "*",
  ...
  for (var in c("Scope12", "SBTiTarget1","Scope12:SBTiTarget1")) {
  ...
  # Append results to the summary dataframe
  for (var in c("Scp12Int", "SBTiTarget1", "Scp12Int:SBTiTarget1")) {
  ...
  # Append results to the summary dataframe
  for (var in c("Scp12YoY", "SBTiTargetYoY1","Scp12YoY:SBTiTargetYoY1")) {
...
results_df1 <- results_df1 %>%
  mutate(
    ...
    `Scope12:SBTiTarget1` = ifelse(!is.na(`Coefficient_Scope12:SBTiTarget1`), paste0(sprintf("%.3f", `Coefficient_Scope12:SBTiTarget1`), `Significance_Scope12:SBTiTarget1`), NA),
    SBTiTarget1 = ifelse(!is.na(Coefficient_SBTiTarget1), paste0(sprintf("%.3f", Coefficient_SBTiTarget1), Significance_SBTiTarget1), NA)
  ) %>%
  select(Country,Scope12, `Scope12:SBTiTarget1`, SBTiTarget1)
...
# Save output in a text file
writeLines(summary1, paste0("7.41_Summary.tex"))
...
results_df2 <- results_df2 %>%
  mutate(
    ...
    `Scp12Int:SBTiTarget1` = ifelse(!is.na(`Coefficient_Scp12Int:SBTiTarget1`), paste0(sprintf("%.3f", `Coefficient_Scp12Int:SBTiTarget1`), `Significance_Scp12Int:SBTiTarget1`), NA),
    SBTiTarget1 = ifelse(!is.na(Coefficient_SBTiTarget1), paste0(sprintf("%.3f", Coefficient_SBTiTarget1), Significance_SBTiTarget1), NA)
  ) %>%
  select(Country,Scp12Int, `Scp12Int:SBTiTarget1`, SBTiTarget1)
...
# Save output in a text file
writeLines(summary2, paste0("7.42_Summary.tex"))
...
results_df3 <- results_df3 %>%
  mutate(
    ...
    `Scp12YoY:SBTiTargetYoY1` = ifelse(!is.na(`Coefficient_Scp12YoY:SBTiTargetYoY1`), paste0(sprintf("%.3f", `Coefficient_Scp12YoY:SBTiTargetYoY1`), `Significance_Scp12YoY:SBTiTargetYoY1`), NA),
    SBTiTargetYoY1 = ifelse(!is.na(Coefficient_SBTiTargetYoY1), paste0(sprintf("%.3f", Coefficient_SBTiTargetYoY1), Significance_SBTiTargetYoY1), NA)
  ) %>%
  select(Country,Scp12YoY, `Scp12YoY:SBTiTargetYoY1`, SBTiTargetYoY1)
...
# Save output in a text file
writeLines(summary3, paste0("7.43_Summary.tex"))